...
public class UserCacheManager {
   ...
   // private static final String kChatUserPic = "ChatUserPic";// 头像Url
    private static final String kChatUserId = "buddy";// 环信ID
    private static final String kChatUserNick = "nickname";// 昵称
    private static final String kChatUserPic = "headerUrl";// 头像Url
    ...
    public static List<UserCacheInfo> getAll() {
        Dao<UserCacheInfo, Integer> dao = null;
        try {
            dao = SqliteHelper.getInstance().getUserDao();
     ...
     * @return
     */
    public static void save(Map<String, Object> ext,String id) {
        Log.e("ext",JsonUtil.toJson(ext));
        if (ext == null) return;
        try {
            String userId = id+"";
    ...
    public static void setMsgExt(EMMessage msg) {
        ...
        if (user!=null&&user.getUserId()!=null&&user.getNickName()!=null&&user.getAvatarUrl()!=null){
            ...
            msg.setAttribute(kChatUserId, PreferenceUtil.getString(ConstantsBean.Spell,ConstantsBean.FROM_ID));
            msg.setAttribute(kChatUserNick, PreferenceUtil.getString(ConstantsBean.Spell,ConstantsBean.FROM_NICHENG));
            msg.setAttribute(kChatUserPic,PreferenceUtil.getString(ConstantsBean.Spell,ConstantsBean.FROM_AVATER));
        }

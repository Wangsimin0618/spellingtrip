...
import com.spellingtrip.example.huanxin.db.DemoDBManager;
import com.spellingtrip.example.huanxin.db.InviteMessgeDao;
import com.spellingtrip.example.huanxin.db.UserDao;
import com.spellingtrip.example.huanxin.domain.EmojiconExampleGroupData;
import com.spellingtrip.example.huanxin.domain.InviteMessage;
import com.spellingtrip.example.huanxin.domain.RobotUser;
import com.spellingtrip.example.huanxin.parse.UserProfileManager;
import com.spellingtrip.example.huanxin.receiver.CallReceiver;
import com.spellingtrip.example.huanxin.receiver.HeadsetReceiver;
import com.spellingtrip.example.huanxin.utils.PreferenceManager;
...
//        options.setImPort(31097);
                ...
                if(isVideoCalling){
                   // intent = new Intent(appContext, VideoCallActivity.class);
                    ...
                    if (chatType == ChatType.Chat) { // single chat message
                        ...
                        intent.putExtra("chatType", Constanthuan.CHATTYPE_SINGLE);
                    } else { // group chat message
                        // message.getTo() is the group id
                        intent.putExtra("userId", message.getTo());
                        if(chatType == ChatType.GroupChat){
                            intent.putExtra("chatType", Constanthuan.CHATTYPE_GROUP);
                        }else{
                            intent.putExtra("chatType", Constanthuan.CHATTYPE_CHATROOM);
                        }
        ...
        connectionListener = new EMConnectionListener() {
            ...
            public void onDisconnected(int error) {
                ...
                if (error == EMError.USER_REMOVED) {
                    onUserException(Constanthuan.ACCOUNT_REMOVED);
                } else if (error == EMError.USER_LOGIN_ANOTHER_DEVICE) {
                    onUserException(Constanthuan.ACCOUNT_CONFLICT);
                } else if (error == EMError.SERVER_SERVICE_RESTRICTED) {
                    onUserException(Constanthuan.ACCOUNT_FORBIDDEN);
                } else if (error == EMError.USER_KICKED_BY_CHANGE_PASSWORD) {
                    onUserException(Constanthuan.ACCOUNT_KICKED_BY_CHANGE_PASSWORD);
                } else if (error == EMError.USER_KICKED_BY_OTHER_DEVICE) {
                    onUserException(Constanthuan.ACCOUNT_KICKED_BY_OTHER_DEVICE);
                }
            }
    ...
    public void goConference(String confId, String password, String extension) {
        ...
        try {
            ...
            inviter = jsonObj.optString(Constanthuan.EXTRA_CONFERENCE_INVITER);
            groupId = jsonObj.optString(Constanthuan.EXTRA_CONFERENCE_GROUP_ID);
    ...
    class MyGroupChangeListener implements EMGroupChangeListener {
        ...
        public void onInvitationReceived(String groupId, String groupName, String inviter, String reason) {
            ...
            broadcastManager.sendBroadcast(new Intent(Constanthuan.ACTION_GROUP_CHANAGED));
        }
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactAdded(String username) {
            ...
            broadcastManager.sendBroadcast(new Intent(Constanthuan.ACTION_CONTACT_CHANAGED));
    	...
    	messageListener = new EMMessageListener() {
			...
			public void onMessageReceived(List<EMMessage> messages) {
			    for (EMMessage message : messages) {
			        ...
			        Log.e("userid",userid+"");
                    UserCacheManager.save(message.ext(),userid);
                    // in background, do not refresh UI, notify it in notification bar
                    if (!easeUI.hasForegroundActivies()) {
                        getNotifier().notify(message);
                    }
                    /*String userName = message.getStringAttribute(ConstantsBean.FROM_NICHENG, "");
                    String userPic = message.getStringAttribute(ConstantsBean.FROM_AVATER, "");
                    ...
                    // 存入db
                    UserDao dao = new UserDao(CustomApplication.context);
                    ...
                    // ******************扩展信息处理完成**********************
                    // 判断一下是否是会议邀请
                   /* String confId = message.getStringAttribute(Constanthuan.MSG_ATTR_CONF_ID, "");
                    if(!"".equals(confId)){
                        String password = message.getStringAttribute(Constanthuan.MSG_ATTR_CONF_PASS, "");
                        String extension = message.getStringAttribute(Constanthuan.MSG_ATTR_EXTENSION, "");
                        goConference(confId, password, extension);
                    }*/
                    // in background, do not refresh UI, notify it in notification bar
                  /*  if(!easeUI.hasForegroundActivies()){
            ...
            public void onMessageRecalled(List<EMMessage> messages) {
                for (EMMessage msg : messages) {
                    ...
                    msgNotification.setAttribute(Constanthuan.MESSAGE_TYPE_RECALL, true);

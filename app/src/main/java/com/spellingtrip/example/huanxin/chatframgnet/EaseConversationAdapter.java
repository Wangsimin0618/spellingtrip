...
import com.spellingtrip.example.liaotian.UserCacheInfo;
import com.spellingtrip.example.liaotian.UserCacheManager;
import com.spellingtrip.example.utils.ChatUserInfoUtils;
        ...
        if (conversation!=null){
           ...
           /*  if (conversation.getType() == EMConversationType.GroupChat) {
               String groupId = conversation.conversationId();
                ...
                /**设置头像**/
                if (conversation.getType() != EMConversationType.GroupChat&&conversation.getType() != EMConversationType.ChatRoom){
                    UserCacheInfo user=UserCacheManager.get(username);
                    if (user!=null){
                        if(user != null&&user.getAvatarUrl()!= null){
                            try {
                                //int avatarResId = Integer.parseInt(user.getAvatarUrl());
                                Glide.with(mContext).load(user.getAvatarUrl()).diskCacheStrategy(DiskCacheStrategy.ALL).into(holder.avatar);
                            } catch (Exception e) {
                                //use default avatar
                                Glide.with(mContext).load(user.getAvatarUrl()).diskCacheStrategy(DiskCacheStrategy.ALL)
                                        .placeholder(R.drawable.ease_default_avatar).into(holder.avatar);
                            }
                        }else{
                            Glide.with(mContext).load(R.drawable.ease_default_avatar).into(holder.avatar);
                        }
                        /**设置昵称**/

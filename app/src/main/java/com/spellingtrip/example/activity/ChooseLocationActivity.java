...
public class ChooseLocationActivity extends AppCompatActivity implements LocationSource,
   ...
   private String searchKey = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_amapaddress);
        progDialog = new ProgressDialog(this);
        showDialog();
        onShow();
        mapView = (MapView) findViewById(R.id.chooseLocationMap);
        tvGetAddress = (TextView) findViewById(R.id.tvGetChooseAddress);
        tv_lantitle = (TextView) findViewById(R.id.tv_lantitle);
        tv_lantitle.setText("选择地点");
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (newText.length() > 0) {
                    ...
                    if (!TextUtils.isEmpty(nowCity)){
                        inputquery = new InputtipsQuery(newText, nowCity);
                    }else {
                        inputquery = new InputtipsQuery(newText, "北京市");
                    }

                    Inputtips inputTips = new Inputtips(ChooseLocationActivity.this, inputquery);
...
//        Log.i("MY", "onLocationChanged");
        if (mListener != null && amapLocation != null) {
            if (amapLocation != null
                ...
                isInputKeySearch = false;
                searchText.setHint(amapLocation.getCity()+amapLocation.getDescription());
            } else {
    ...
    public void onRegeocodeSearched(RegeocodeResult result, int rCode) {
        if (rCode == AMapException.CODE_AMAP_SUCCESS) {
            if (result != null && result.getRegeocodeAddress() != null
                ...
                county = result.getRegeocodeAddress().getDistrict();
                doSearchQuery();
            }else {
            }
        } else {
            Toast.makeText(ChooseLocationActivity.this, "error code is " + rCode, Toast.LENGTH_SHORT).show();
        }
    }
        ...
        if (resultCode == AMapException.CODE_AMAP_SUCCESS) {
            if (poiResult != null && poiResult.getQuery() != null) {
                if (poiResult.getQuery().equals(query)) {
                    ...
                    longitube=poiResult.getPois().get(0).getLatLonPoint().getLongitude()+"";
                }else {
                }
            } else {
                Toast.makeText(ChooseLocationActivity.this, "无搜索结果", Toast.LENGTH_SHORT).show();
            }
        }
    }
...
dismissDialog();
    ...
    Inputtips.InputtipsListener inputtipsListener = new Inputtips.InputtipsListener() {
        ...
        public void onGetInputtips(List<Tip> list, int rCode) {
            if (rCode == AMapException.CODE_AMAP_SUCCESS) {// 正确返回
                ...
                Toast.makeText(ChooseLocationActivity.this, "erroCode " + rCode, Toast.LENGTH_SHORT).show();
            }
        }
    };
    ...
    private void searchPoi(Tip result) {
        taskaddress = result.getAddress();
        latitude = result.getPoint().getLatitude() + "";
        longitube = result.getPoint().getLongitude() + "";
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.tvGetChooseAddress:
               String address= searchText.getText().toString().trim();
                String location="";
               if (!TextUtils.isEmpty(address)){
                   location=address;
               }else {
                   location= searchText.getHint().toString().trim();
               }
                setResult(RESULT_OK, new Intent().putExtra("province", province).putExtra("city", city).putExtra("county", county)
                        .putExtra("latitude", latitude).putExtra("longitube", longitube).putExtra("address", location));
                finish();
                break;
            case R.id.iv_left:
                finish();
